...
function create_D_vecs(u_vec, t_n, h, C_H) # returns Dplusminus and Dminusplus vectors
    ...
    return (Dplusminus, Dminusplus, Dplusplus, Dminusminus) # removed C_H factor to match the paper
end
...
function create_Dg_mat(v, t_n, N_h, h, C_H) # verified is correct
    ...
    Dg_mat += tridiag
    Dg_mat *= 2/h
    return C_H .* Dg_mat # added C_H factor to match the paper
end
...
function tranport_matrix(U, t_n, N_h, h, C_H) # verified!
    ...
    TU_mat += tridiag
    TU_mat *= 2/h # not by h
    return C_H .* TU_mat # added C_H factor to match the paper
end
